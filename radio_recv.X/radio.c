...
unsigned char fix_radio_packet[RADIO_MAX_PACKET_LENGTH];
...
unsigned char radio_check_received(void) {
    if (nIRQ == 0) {
        ...
        if (si4455_cmd.GET_INT_STATUS.PH_PEND & SI4455_CMD_GET_INT_STATUS_REP_PACKET_RX_PEND_BIT) {
            si4455_read_rx_fifo(radio_configuration.radio_packet_length, fix_radio_packet);
            return 1;
        }
...
unsigned char radio_check_received_variable_packet(void) {
    if (nIRQ == 0) {
        ...
        if (si4455_cmd.GET_INT_STATUS.PH_PEND & SI4455_CMD_GET_INT_STATUS_REP_PACKET_RX_PEND_BIT) {
            si4455_fifo_info(0);
            si4455_read_rx_fifo(si4455_cmd.FIFO_INFO.RX_FIFO_COUNT, fix_radio_packet);
            return 1;
        }
